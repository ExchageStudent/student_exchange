...
class WriteReportStep6Fragment : Fragment() {
        ...
        RetrofitInstance.reportApi.getReportStage(stageDto).enqueue(object :
            ...
            override fun onResponse(call: Call<StageResponseDto>, response: Response<StageResponseDto>) {
                if (response.isSuccessful) {
                    ...
                    if (stageData != null) {
                        Log.d("WriteReportStep6", "Report stage data: $stageData")
                    } else {
                        Log.e("WriteReportStep6", "Stage data is null")
                    }
                } else {
                    Log.e("WriteReportStep6", "Failed to fetch stage: ${response.errorBody()?.string()}")
                }
            }

            override fun onFailure(call: Call<StageResponseDto>, t: Throwable) {
                Log.e("WriteReportStep6", "API call failed: ${t.message}", t)
            }
        })
    }
    ...
    private fun sendSelectedOptionsToServer() {
        ...
        val stageOrder = 5 // 현재 단계에 맞게 stageOrder를 설정
        ...
        Log.d("WriteReportStep6", "Stage request data: $stageRequest")
    ...
    private fun goToNextStep(report: Report) {
        val sixthStep = WriteReportStep7Fragment()
        val bundle = Bundle()
